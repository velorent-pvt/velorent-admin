create or replace function public.get_customer_funnel_dropoff_customers(
...
begin
  ...
  if p_stage_index not between 1 and 10 then
  ...
  qualified as (
    ...
    where progression.stage_index = p_stage_index
      and (
        ...
        or not exists (
          select 1
          from progression next_stage
          where next_stage.customer_id = progression.customer_id
            and next_stage.stage_index = p_stage_index + 1
        )
      )
  )
  select
...
revoke all on function public.get_customer_funnel_dropoff_customers(
  timestamptz, timestamptz, integer
) from public, anon;
grant execute on function public.get_customer_funnel_dropoff_customers(
  timestamptz, timestamptz, integer
) to authenticated;
